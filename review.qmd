...
def simpleTransducerFunction(inputContrast, c50, RMax):
 ...
 # The exponent can also vary - here we fix it to 2. Varying it produces different amplitude spectra.
  ...
  output=RMax*(inputContrast**expnt)/(inputContrast**expnt + c50**expnt) # Here we apply the hyperbolic ratio function
...
def doubleTransducerFunction(inputContrast, maskContrast, c50, RMax):
 # This version has two inputs, but the second input is purely suppressive
  ...
  # Changing this equation can produce a different pattern of intermodulation terms in Figure 5. For example, use the equation below to include both mask and probe in the numerator.
...
plt.tight_layout()
plt.savefig('Figures/1_centresurround.png', dpi=300)
...
axs[2].grid()
plt.tight_layout()
plt.savefig('Figures/2_linearsystem.png', dpi=300)
...
axs[2].grid()
plt.tight_layout()
plt.savefig('Figures/3_fullwaverectification.png', dpi=300)
...
axs[2].grid()
plt.tight_layout()
plt.savefig('Figures/4_transducer.png', dpi=300)
...
axs[2].grid()
plt.tight_layout()
plt.savefig('Figures/5_twoinputs.png', dpi=300)
...
axs.grid()
plt.tight_layout()
plt.savefig('Figures/6_sweepplot.png', dpi=300)
...
# To save the figure
plt.savefig('Figures/7_clinicalplot.png', dpi=300)
...
![The perceived contrast of a stimulus depends on its context. A high contrast surround reduces the apparent contrast of the central 'probe' region.](Figures/1_centresurround.png){#fig-centresurround}
:::
...
::: {.content-visible when-format="docx"}
![Illustration of a sinusoidal input signal (left), its Fourier spectrum (middle), and how the amplitude of the first harmonic component increases with contrast (right). These calculations assume an entirely linear system.](Figures/2_linearsystem.png){#fig-linear}
:::
::: {.content-visible unless-format="docx"}
![Illustration of a sinusoidal input signal (left), its Fourier spectrum (middle), and how the amplitude of the first harmonic component increases with contrast (right). These calculations assume an entirely linear system.](Figures/2_linearsystem.png){#fig-linear}
:::
...
::: {.content-visible when-format="docx"}
![Illustration of a full-wave rectified sinusoid (left), its Fourier spectrum (middle), and how the amplitude of the second harmonic component increases with contrast (right).](Figures/3_fullwaverectification.png){#fig-fullwaverectification}
:::
::: {.content-visible unless-format="docx"}
![Illustration of a full-wave rectified sinusoid (left), its Fourier spectrum (middle), and how the amplitude of the second harmonic component increases with contrast (right).](Figures/3_fullwaverectification.png){#fig-fullwaverectification}
...
::: {.content-visible when-format="docx"}
![Demonstration of how two inputs of different frequencies are processed by a nonlinear transducer. Additional harmonics are apparent in the Fourier spectrum (middle panel), and the contrast response function is suppressed (right panel). The effects of using different nonlinearities can be explored in the accompanying code.](Figures/5_twoinputs.png){#fig-twoinputs}
:::
::: {.content-visible unless-format="docx"}
![Demonstration of how two inputs of different frequencies are processed by a nonlinear transducer. Additional harmonics are apparent in the Fourier spectrum (middle panel), and the contrast response function is suppressed (right panel). The effects of using different nonlinearities can be explored in the accompanying code.](Figures/5_twoinputs.png){#fig-twoinputs}
:::
