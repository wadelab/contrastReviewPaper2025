...
simpleTransducerFunction <- function(inputContrast, c50, RMax){
 ...
 # The exponent can also vary - here we fix it to 2
  ...
  inputContrast <- abs(inputContrast)
  output <- RMax*(inputContrast^expnt)/(inputContrast^expnt + c50^expnt)
  return(output)
}

doubleTransducerFunction <- function(inputContrast, maskContrast, c50, RMax){
 # This version has two inputs, but the second input is purely suppressive
  expnt <- 2
  inputContrast <- abs(inputContrast)
  maskContrast <- abs(maskContrast)

  output <- RMax*(inputContrast^expnt)/(inputContrast^expnt + maskContrast^expnt + c50^expnt)
  return(output)  
}

create_grating <- function(size, frequency, phase, contrast){
    x <- matrix(rep(seq(-1,1,length=size),size),nrow=size,ncol=size)
    y <- t(x)
    grating <- sin(2 * pi * frequency * sqrt(x^2 + y^2) + phase)
    grating <- (grating + 1) / 2  # Normalize to [0, 1]
    grating <- grating * contrast + (1 - contrast) / 2  # Adjust contrast 
  return(grating)
}

create_circular_mask <- function(size){
    x <- matrix(rep(seq(-1,1,length=size),size),nrow=size,ncol=size)
    y <- t(x)
    radius <- sqrt(x^2 + y^2)
    mask <- sqrt(x**2 + y**2) <= 0.3
  return(mask)
}
...
image_low_contrast <- grating_surround_low
image_low_contrast[mask] <- grating_center[mask]
image_high_contrast <- grating_surround_high
image_high_contrast[mask] <- grating_center[mask]

pdf('Figures/1_centresurround.pdf',width=12,height=6.5)

par(mfcol=c(1,2))

plot(x=NULL,y=NULL,xlim=c(0,1),ylim=c(0,1),axes=FALSE, ann=FALSE)
image(image_low_contrast,col=gray.colors(256),add=TRUE,useRaster=TRUE)
title(main='Low-Contrast Background')

plot(x=NULL,y=NULL,xlim=c(0,1),ylim=c(0,1),axes=FALSE, ann=FALSE)
image(image_high_contrast,col=gray.colors(256),add=TRUE,useRaster=TRUE)
title(main='High-Contrast Background')
...
inputFrequency_Hz <- 5 # We simulate a system with on input at many different contrasts
contrastRange <- (10^seq(0,1.5,length=20))/100 # Equally spaced log contrast between 1 and 10^1.5
...
support_S <- seq(0,duration_S,length=duration_S*digitisationRate_Hz)

initialSignal <- NULL # Create lists to hold the initial contrast representation and the result of passign it through a simple Naka-Rushton function
transducedSignal <- NULL

for (contrastIndex in 1:length(contrastRange)){
  
  inputModulation <- sin(2 * pi * inputFrequency_Hz * support_S) * contrastRange[contrastIndex]

  FT_Signal <- fft(inputModulation) / length(inputModulation)
 
  inputAmplitude <- abs(FT_Signal[inputFrequency_Hz+1])
  initialSignal[contrastIndex] <- inputAmplitude
  ...
  FT_transducedModulation <- fft(transducedModulation)/ length(transducedModulation)
  transducedAmplitude <- abs(FT_transducedModulation[1 + inputFrequency_Hz*2])
  transducedSignal[contrastIndex] <- transducedAmplitude
}

frequencySupport <- seq(0,freqCutoff_Hz*duration_S+1) # We only want to plot the frequencies in the FFT up to this point
  
#Make a figure with three subplots

pdf('Figures/2_linearsystem.pdf',width=12,height=4)
par(mfrow=c(1,3))

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(-0.3,0.3))   
  axis(1, at=seq(0,1,0.5), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(-3,3,1)/10, tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,1,0.5), side = 1, at=seq(0,1,0.5), line=0.2)     # add the tick labels
  mtext(text = seq(-3,3,1)/10, side = 2, at=seq(-3,3,1)/10, line=0.2, las=1)  
title(xlab="Time (s)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(main="Input sine wave", cex.main=1.5)

lines(support_S, inputModulation, lwd=3)

# Plot the absolute values of the FFT in the second axis as a bar plot

  plot(frequencySupport[2:length(frequencySupport)]-0.5,abs(FT_Signal[2:length(frequencySupport)]),type='s',lwd=3,axes=FALSE, ann=FALSE, xlim=c(0,30), ylim=c(0,0.16))
  axis(1, at=seq(0,30,10), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,0.16,0.04), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,30,10), side = 1, at=seq(0,30,10), line=0.2)     # add the tick labels
  mtext(text = seq(0,0.16,0.04), side = 2, at=seq(0,0.16,0.04), line=0.2, las=1)  
title(xlab="Frequency (Hz)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Magnitude (a.u.)", col.lab=rgb(0,0,0), line=2.6, cex.lab=1.5)
title(main="Fourier spectrum", cex.main=1.5)

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,32), ylim=c(0,0.16))   
  axis(1, at=seq(0,32,4), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,0.16,0.04), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,32,4), side = 1, at=seq(0,32,4), line=0.2)     # add the tick labels
  mtext(text = seq(0,0.16,0.04), side = 2, at=seq(0,0.16,0.04), line=0.2, las=1)  
title(xlab="Contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Amplitude (a.u.)", col.lab=rgb(0,0,0), line=2.6, cex.lab=1.5)
title(main="Linear computation stage", cex.main=1.5)

lines(contrastRange*100,initialSignal,lwd=3)

dev.off()

```

```{r fullwaverectification}
...
for (contrastIndex in 1:length(contrastRange)){
  ...
  fullWaveRectifiedSignal <- abs(inputModulation)
  ...
  FT_transducedModulation <- fft(fullWaveRectifiedSignal) / length(fullWaveRectifiedSignal)
 
  transducedAmplitude <- abs(FT_transducedModulation[2*inputFrequency_Hz+1])
  transducedSignal[contrastIndex] <- transducedAmplitude
...
pdf('Figures/3_fullwaverectification.pdf',width=12,height=4)
par(mfrow=c(1,3))

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,0.35))   
  axis(1, at=seq(0,1,0.5), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,3,1)/10, tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,1,0.5), side = 1, at=seq(0,1,0.5), line=0.2)     # add the tick labels
  mtext(text = seq(0,3,1)/10, side = 2, at=seq(0,3,1)/10, line=0.2, las=1)  
title(xlab="Time (s)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(main="Rectified signal", cex.main=1.5)

lines(support_S, fullWaveRectifiedSignal, lwd=3)

# Plot the absolute values of the FFT in the second axis as a bar plot
  plot(frequencySupport[2:length(frequencySupport)]-0.5,abs(FT_transducedModulation[2:length(frequencySupport)]),type='s',lwd=3,axes=FALSE, ann=FALSE, xlim=c(0,30), ylim=c(0,0.08))
  axis(1, at=seq(0,30,10), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,0.08,0.02), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,30,10), side = 1, at=seq(0,30,10), line=0.2)     # add the tick labels
  mtext(text = seq(0,0.08,0.02), side = 2, at=seq(0,0.08,0.02), line=0.2, las=1)  
title(xlab="Frequency (Hz)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Magnitude (a.u.)", col.lab=rgb(0,0,0), line=2.6, cex.lab=1.5)
title(main="Fourier spectrum", cex.main=1.5)

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,32), ylim=c(0,0.08))   
  axis(1, at=seq(0,32,4), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,0.08,0.02), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,32,4), side = 1, at=seq(0,32,4), line=0.2)     # add the tick labels
  mtext(text = seq(0,0.08,0.02), side = 2, at=seq(0,0.08,0.02), line=0.2, las=1)  
title(xlab="Contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Amplitude (a.u.)", col.lab=rgb(0,0,0), line=2.6, cex.lab=1.5)
title(main="Contrast response function", cex.main=1.5)

lines(contrastRange*100,transducedSignal,lwd=3)
...
pdf('Figures/4_transducer.pdf',width=12,height=4)
par(mfrow=c(1,3))

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,1), ylim=c(0,10))   
  axis(1, at=seq(0,1,0.5), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,10,2), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,1,0.5), side = 1, at=seq(0,1,0.5), line=0.2)     # add the tick labels
  mtext(text = seq(0,10,2), side = 2, at=seq(0,10,2), line=0.2, las=1)  
title(xlab="Time (s)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(main="Transduced signal", cex.main=1.5)

lines(support_S, transducedModulation, lwd=3)

# Plot the absolute values of the FFT in the second axis as a bar plot

  plot(frequencySupport[2:length(frequencySupport)]-0.5,abs(FT_transducedModulation[2:length(frequencySupport)]),type='s',lwd=3,axes=FALSE, ann=FALSE, xlim=c(0,30), ylim=c(0,2))
  axis(1, at=seq(0,30,10), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,2,0.5), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,30,10), side = 1, at=seq(0,30,10), line=0.2)     # add the tick labels
  mtext(text = seq(0,2,0.5), side = 2, at=seq(0,2,0.5), line=0.2, las=1)  
title(xlab="Frequency (Hz)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Magnitude (a.u.)", col.lab=rgb(0,0,0), line=2.6, cex.lab=1.5)
title(main="Fourier spectrum", cex.main=1.5)

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(0,32), ylim=c(0,2))   
  axis(1, at=seq(0,32,4), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,2,0.5), tck=0.01, lab=F, lwd=2)
  mtext(text = seq(0,32,4), side = 1, at=seq(0,32,4), line=0.2)     # add the tick labels
  mtext(text = seq(0,2,0.5), side = 2, at=seq(0,2,0.5), line=0.2, las=1)  
...
contrastRange <- (10^seq(0,1.5,length=20))/100 # Equally spaced log contrast between 1 and 10^1.5
maskFrequency_Hz <- 7
maskContrast <- max(contrastRange)
...
for (contrastIndex in 1:length(contrastRange)){
  ...
  inputModulation <- sin(2 * pi * inputFrequency_Hz * support_S) * contrastRange[contrastIndex]
  maskModulation <- sin(2 * pi * maskFrequency_Hz * support_S) * maskContrast
  ...
  FT_transducedModulation <- fft(transducedModulation)/ length(transducedModulation)
  transducedAmplitude <- abs(FT_transducedModulation[1+inputFrequency_Hz*2])
  maskedSignal[contrastIndex] <- transducedAmplitude
}
...
pdf('Figures/5_twoinputs.pdf',width=12,height=4)
par(mfrow=c(1,3))
...
lines(contrastRange*100,transducedSignal,lwd=3)
lines(contrastRange*100,maskedSignal,col='orange',lwd=3)
...
internalNoise <- 1
inputFrequency_Hz <- 5 # We simulate a system with on input at many different contrasts
contrastRange <- (10^seq(1,1.9,length=nLevels))/100  # Equally spaced log contrast between 1 and 10^1.9
duration_S <- 1 # All variables in this code have an _[units] where possible
digitisationRate_Hz <- 1000
freqCutoff_Hz <- 30
...
transducedSignal <- NULL
for (contrastIndex in 1:length(contrastRange)){
  inputModulation <- sin(2 * pi * inputFrequency_Hz * support_S) * contrastRange[contrastIndex] + runif(length(support_S))*internalNoise

  FT_Signal <- fft(inputModulation) / length(inputModulation)

  responseAmplitude <- abs(FT_Signal[inputFrequency_Hz+1])
  initialSignal[contrastIndex] <- responseAmplitude
  transducedSignal[contrastIndex] <- simpleTransducerFunction(responseAmplitude, 0.05, 10)
}
...
X = log10(contrastRange[1:4])
X2 = log10((10^seq(0.1,1.1,length=nLevels))/100)
y <- transducedSignal[1:4]
reg <- lm(y ~ X)
x_intercept <- 10 ^ (-reg$coefficients[1] / reg$coefficients[2])
Yfit <- reg$coefficients[2]*X + reg$coefficients[1]
Yextrap <- reg$coefficients[2]*X2 + reg$coefficients[1]

pdf('Figures/6_sweepplot.pdf',width=6,height=6)

  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-2,0), ylim=c(0,10))   
  axis(1, at=seq(-2,0,1), tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=seq(0,10,2), tck=0.01, lab=F, lwd=2)
  mtext(text = c(0.01,0.1,1), side = 1, at=seq(-2,0,1), line=0.2)     # add the tick labels
  mtext(text = seq(0,10,2), side = 2, at=seq(0,10,2), line=0.2, las=1)  
title(xlab="Contrast", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(ylab="Amplitude (a.u.)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
title(main=paste("Simulated sweep VEP at",nLevels,"different contrasts"), cex.main=1.5)

points(log10(contrastRange),transducedSignal,pch=19,col='cornflowerblue',cex=2)
points(log10(x_intercept),0,pch=19,col='red',cex=2)

lines(X,Yfit,lwd=3,col='red')
lines(X2,Yextrap,lwd=3,lty=2,col='red')

legend(-2,10,c('Linear fit','Threshold'),lty=c(1,NA),pch=c(NA,19),lwd=3,col='red',box.lwd=2,cex=1.5)
...
# Parameters
finecontrastsdB <- seq(-18,36,1)
finecontrasts <- 10^(finecontrastsdB / 20)
ticklocsx <- seq(-18,36,6)
ticklabelsx <- c('1/8', '1/4', '1/2', 1, 2, 4, 8, 16, 32, 64)

pdf('Figures/7_clinicalplot.pdf',width=12,height=12)
par(mfrow=c(2,2))

epilepsyparams <- matrix(0,nrow=2,ncol=4) 
epilepsyparams[1,] <- c(3.4, 0.15, 1.4, 1)  # controls
epilepsyparams[2,] <- c(5.3, 0.23, 1.4, 1)  # patients

# Calculate responses
respEPIcontrols <- epilepsyparams[1,1] * ((finecontrasts/100)^epilepsyparams[1,3] /
                (epilepsyparams[1,2]^epilepsyparams[1,3] + (finecontrasts/100)^epilepsyparams[1,3])) + epilepsyparams[1,4]

respEPIpatients <- epilepsyparams[2,1] * ((finecontrasts/100)^epilepsyparams[2,3] /
                (epilepsyparams[2,2]^epilepsyparams[2,3] + (finecontrasts/100)^epilepsyparams[2,3])) + epilepsyparams[2,4]

ticklocsy <- seq(0,6,2)
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-18,36), ylim=c(0,6))   
  ...
  title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
  title(main="(a) Epilepsy", cex.main=1.5)

lines(finecontrastsdB, respEPIpatients, lwd=4, col='blue')
lines(finecontrastsdB, respEPIcontrols, lwd=4, col='black', lty=2)

legend(-18,6,c('Epilepsy patients','Control participants'),lwd=4,lty=1:2,col=c('blue','black'),box.lwd=2,cex=1.5)



amblyparams <- matrix(0,nrow=2,ncol=4) 
amblyparams[1,] <- c(0.39, 37.2, 1.4, 0.22)  # amblyopic eye
amblyparams[2,] <- c(2.33, 111.69, 1.4, 0.22)  # fellow eye

# Calculate responses
respambly <- amblyparams[1,1] * ((finecontrasts)^amblyparams[1,3] /
                (amblyparams[1,2] + (finecontrasts)^amblyparams[1,3])) + amblyparams[1,4]

respfellow <- amblyparams[2,1] * ((finecontrasts)^amblyparams[2,3] /
                (amblyparams[2,2] + (finecontrasts)^amblyparams[2,3])) + amblyparams[2,4]

ticklocsy <- seq(0,2,0.5)
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-18,36), ylim=c(0,2))   
  axis(1, at=ticklocsx, tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=ticklocsy, tck=0.01, lab=F, lwd=2)
  mtext(text = ticklabelsx, side = 1, at=ticklocsx, line=0.2)     # add the tick labels
  mtext(text = ticklocsy, side = 2, at=ticklocsy, line=0.2, las=1)  
  title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)     
  title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
  title(main="(b) Amblyopia", cex.main=1.5)

lines(finecontrastsdB, respambly, lwd=4, col='red')
lines(finecontrastsdB, respfellow, lwd=4, col='black', lty=2)

legend(-18,2,c('Amblyopic eye','Fellow eye'),lwd=4,lty=1:2,col=c('red','black'),box.lwd=2,cex=1.5)


# Autism and control groups for the autism plot

autismparams <- matrix(0,nrow=2,ncol=4) 
autismparams[1,] <- c(0.6253, 0.2701, 125550, 5.6199)  # ASD group
autismparams[2,] <- c(1.0131, 0.3369, 3589900, 6.8622)  # control group

# Calculate responses
respASD <- (autismparams[1,1] * (finecontrasts^autismparams[1,4]) /
           (autismparams[1,3] + finecontrasts^autismparams[1,4])) + autismparams[1,2]
respCON <- (autismparams[2,1] * (finecontrasts^autismparams[2,4]) /
           (autismparams[2,3] + finecontrasts^autismparams[2,4])) + autismparams[2,2]

ticklocsy <- seq(0,1.5,0.5)
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-18,36), ylim=c(0,1.5))   
  axis(1, at=ticklocsx, tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=ticklocsy, tck=0.01, lab=F, lwd=2)
  mtext(text = ticklabelsx, side = 1, at=ticklocsx, line=0.2)     # add the tick labels
  mtext(text = ticklocsy, side = 2, at=ticklocsy, line=0.2, las=1)  
  title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)    
  title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
  title(main="(c) Autism", cex.main=1.5)

lines(finecontrastsdB, respASD, lwd=4, col='darkgreen')
lines(finecontrastsdB, respCON, lwd=4, col='black', lty=2)

legend(-18,1.5,c('ASD children','Control children'),lwd=4,lty=1:2,col=c('darkgreen','black'),box.lwd=2,cex=1.5)


# PD parameters for Drosophila from Afsari et al HMG 2014

PDparams2F1 <- matrix(0,nrow=2,ncol=4) 
PDparams2F1[1,] <- c(22, 0.7, 2, 1)  # controls
PDparams2F1[2,] <- c(24, 0.4, 2, 1)  # hLRRK2

PDparams1F1 <- matrix(0,nrow=2,ncol=4) 
PDparams1F1[1,] <- c(104, 0.76, 2, 1)  # controls
PDparams1F1[2,] <- c(170, 0.382, 2, 1)  # hLRRK2
...
respParkControls <- PDparams2F1[1,1] * ((finecontrasts/100)^PDparams2F1[1,3] /
                      (PDparams2F1[1,2]^PDparams2F1[1,3] + (finecontrasts/100)^PDparams2F1[1,3])) + PDparams2F1[1,4]

respParkPatients <- PDparams2F1[2,1] * ((finecontrasts/100)^PDparams2F1[2,3] /
                      (PDparams2F1[2,2]^PDparams2F1[2,3] + (finecontrasts/100)^PDparams2F1[2,3])) + PDparams2F1[2,4]


ticklocsy <- seq(0,20,4)
  plot(x=NULL,y=NULL,axes=FALSE, ann=FALSE, xlim=c(-18,36), ylim=c(0,20))   
  axis(1, at=ticklocsx, tck=0.01, lab=F, lwd=2)     # plot tick marks (no labels)
  axis(2, at=ticklocsy, tck=0.01, lab=F, lwd=2)
  mtext(text = ticklabelsx, side = 1, at=ticklocsx, line=0.2)     # add the tick labels
  mtext(text = ticklocsy, side = 2, at=ticklocsy, line=0.2, las=1)  
  title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)       
  title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
  title(main="(d) Parkinson's", cex.main=1.5)

lines(finecontrastsdB, respParkPatients, lwd=4, col='purple')
lines(finecontrastsdB, respParkControls, lwd=4, col='black', lty=2)

legend(-18,20,c('hLRRK2 transgenic flies','TH+ (control) files'),lwd=4,lty=1:2,col=c('purple','black'),box.lwd=2,cex=1.5)
...
::: {.content-visible when-format="docx"}
![Illustration of a full-wave rectified sinusoid (left), its Fourier spectrum (middle), and how the amplitude of the second harmonic component increases with contrast (right).](Figures/3_fullwaverectification.pdf){#fig-fullwaverectification}
:::
::: {.content-visible unless-format="docx"}
![Illustration of a full-wave rectified sinusoid (left), its Fourier spectrum (middle), and how the amplitude of the second harmonic component increases with contrast (right).](Figures/3_fullwaverectification.pdf){#fig-fullwaverectification}
...
$$
R=R_{\mathrm{max}} \cdot \frac{C_{\mathrm{in}}^n}{C_{50}^n + C_{\mathrm{in}}^n},$${#eq-nakarushton}
...
$$
R=R_{\mathrm{max}} \cdot \frac{C_{\mathrm{in}}^n}{C_{50}^n + C_{\mathrm{in}}^n + C_{\mathrm{mask}}^n},$${#eq-gaincontrol}
...
At least some of this difference is likely due to the relative objectivity and high SNR of the SSVEP technique compared to other methods such as preferential looking, which requires careful measurement of the infant's gaze direction. In slightly older children, other groups have reported electrophysiological correlates of visual acuity that more closely match the behavioural measures [@De-Vries-Khoe1982].
